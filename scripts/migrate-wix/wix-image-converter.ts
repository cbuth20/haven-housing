...
export class WixImageConverter {
  ...
  async migrateImage(
    ...
    try {
      ...
      const fileName = this.generateFileName(propertyId, imageType, index)
  ...
  private extractWixImageId(wixUrl: string): string | null {
    // Handle wix:image://v1/d56adb_abc123~mv2.jpg/...
    const match = wixUrl.match(WIX_IMAGE_PATTERN)
    if (match && match[2]) {
      // Extract just the image ID part (before any # or /)
      const imageId = match[2].split('#')[0].split('/')[0]
      return imageId
    }
    ...
    if (wixUrl.includes('wix:image://')) {
      ...
      if (parts.length >= 2) {
        const imageId = parts[1].split('#')[0].split('/')[0]
        return imageId
      }
    }
  ...
  private generateFileName(
    ...
    return `${propertyId}-${imageType}-${index}-${timestamp}-${randomString}.jpg`
  }
